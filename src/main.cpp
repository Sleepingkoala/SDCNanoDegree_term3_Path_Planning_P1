...
// Calculate the closest waypoint to current x, y position
...
// Return the next waypoint of the closest waypoint
...
// All codes above are from helpers.h
...
int main() {
    ...
    // The max s value before wrapping around the track back to 0; single track length
    ...
    h.onMessage([&map_waypoints_x, &map_waypoints_y, &map_waypoints_s, &map_waypoints_dx, &map_waypoints_dy, &my_car, &log_file](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    //cout << sdata << endl;
        if (length && length > 2 && data[0] == '4' && data[1] == '2') {
            ...
            if (s != "") {
                ...
                if (event == "telemetry") {
                // j[1] is the data JSON object, including self car's localization and state data
		    double car_x = j[1]["x"];
                    double car_y = j[1]["y"];
		    double car_s = j[1]["s"];
		    double car_d = j[1]["d"];
		    ...
		    car_speed *= 0.44704;  // convert mph to m/s, 1 mph = 1.61km/h = 0.447m/s
		    ...
		    /**
		    * TODO: Define a path made of (x,y) points that the car will visit sequentially every .02 seconds
		    **/
		    
		    
		    //*************************** path generation **********************************	
		    //***************************************************************************
		    // step.1 calculate smooth nearby waypoints by simple cubic spline interpolation  
		    ...
		    // take NUM_WAYPOINTS = 5 previous and subsequent waypoints, respectively for smoothness, refer to constants.h
			...
			// correct for wrap in s-area for spline interpolation (should be continuous lines), refer to spline.h
		    ...
		    }  // end smooth preparation for 

		    // do the interpolation, please refer to smoother.h and spline.h
		   double dist_inc = 0.5;  // the distance interval is 0.5 meter
		   ...
		   for (int i = 1; i < num_interpolation_points; i++) {
		    ...
		    interpolated_waypoints_dy = interpolate_points(coarse_waypoints_s, coarse_waypoints_dy, dist_inc, num_interpolation_points);
                    ...
                    //***************************************************************
		    // step.2 compute ego vehicle parameters and states 
		    // Vehicle class requires ego vehicle frenet parameters named s,s_d,s_dd,d,d_d,d_dd.
		    double pos_s, s_dot, s_ddot;
                    double pos_d, d_dot, d_ddot;
                    
		     // other necessary values for determining the parameters above based on future points in previous path
		    double pos_x, pos_y, pos_x2, pos_y2, angle, vel_x1, vel_y1, pos_x3, pos_y3, vel_x2, vel_y2, acc_x, acc_y;

                    // calculate the partial path size
		    int subpath_size = min(PREVIOUS_PATH_POINTS_TO_KEEP, (int)previous_path_x.size());
		    double traj_start_time = subpath_size * PATH_DT;    // PATH_DT = 0.02 second
			...
			s_ddot = 0;
			d_ddot = 0;
		     } else { // if enough previous path points, use previous path data
			    // consider current position to be the last point of previous path to be kept
			    ...
			    // calculate dx, dy vector from the interpolated waypoints in step1, with pos_x,pos_y as reference point;
			    // since interpolated waypoints are  about 1m apart and path points tend to be <0.5m apart, these 
			 ...
			 my_car.d_dd = d_ddot;          // d dot-dot - acceleration in d
                         ...
                         //***************************************************************************
			// step.3 predict from sensor fusion data 
			...
			if (car_just_ahead) cout << "CAR JUST AHEAD!!!" << endl;
                        ...
                        //******************************************************************
			// step.4  choose the best trajectory
  ...
  // We don't need this since not using HTTP but if it's removed the program doesn't compile 
